...
import utils.rcnn.utils as utils_rcnn
...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        scores = utils_rcnn.batch_slice([scores, ix], lambda x, y: tf.gather(x, y),
                                   self.config.IMAGES_PER_GPU)
        deltas = utils_rcnn.batch_slice([deltas, ix], lambda x, y: tf.gather(x, y),
                                   self.config.IMAGES_PER_GPU)
        anchors = utils_rcnn.batch_slice(ix, lambda x: tf.gather(anchors, x),
                                    self.config.IMAGES_PER_GPU,
                                    names=["pre_nms_anchors"])

        # Apply deltas to anchors to get refined anchors.
        # [batch, N, (y1, x1, y2, x2)]
        boxes = utils_rcnn.batch_slice([anchors, deltas],
        ...
        boxes = utils_rcnn.batch_slice(boxes,
        ...
        def nms(normalized_boxes, scores):
            ...
            return proposals
        proposals = utils_rcnn.batch_slice([normalized_boxes, scores], nms,
                                      self.config.IMAGES_PER_GPU)
        return proposals
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    deltas = utils_rcnn.box_refinement_graph(positive_rois, roi_gt_boxes)
...
class DetectionTargetLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        outputs = utils_rcnn.batch_slice(
...
class DetectionLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        detections_batch = utils_rcnn.batch_slice(
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    shape = image.shape
    image, window, scale, padding = utils_rcnn.resize_image(
        image,
        min_dim=config.IMAGE_MIN_DIM,
        max_dim=config.IMAGE_MAX_DIM,
        padding=config.IMAGE_PADDING)
    mask = utils_rcnn.resize_mask(mask, scale, padding)
    ...
    bbox = utils_rcnn.extract_bboxes(mask)
    ...
    if use_mini_mask:
        mask = utils_rcnn.minimize_mask(bbox, mask, config.MINI_MASK_SHAPE)
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    for i in range(overlaps.shape[1]):
        gt = gt_boxes[i]
        overlaps[:, i] = utils_rcnn.compute_iou(
...
#     bg_ids = np.where((rpn_roi_iou_max >= 0.1) & (rpn_roi_iou_max < 0.5))[0]
    ...
    bboxes[pos_ids, roi_gt_class_ids[pos_ids]] = utils_rcnn.box_refinement(
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    ...
    if crowd_ix.shape[0] > 0:
        ...
        # Compute overlaps with crowd boxes [anchors, crowds]
        crowd_overlaps = utils_rcnn.compute_overlaps(anchors, crowd_boxes)
    ...
    overlaps = utils_rcnn.compute_overlaps(anchors, gt_boxes)
...
def data_generator(dataset, config, shuffle=True, augment=True, random_rois=0,
    ...
    anchors = utils_rcnn.generate_pyramid_anchors(config.RPN_ANCHOR_SCALES,
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # Generate Anchors
        self.anchors = utils_rcnn.generate_pyramid_anchors(config.RPN_ANCHOR_SCALES,
                                                      config.RPN_ANCHOR_RATIOS,
                                                      config.BACKBONE_SHAPES,
                                                      config.BACKBONE_STRIDES,
                                                      config.RPN_ANCHOR_STRIDE)
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            molded_image, window, scale, padding = utils_rcnn.resize_image(
                image,
    ...
    def unmold_detections(self, detections, mrcnn_mask, image_shape, window):
        ...
        for i in range(N):
            ...
            full_mask = utils_rcnn.unmold_mask(masks[i], boxes[i], image_shape)
